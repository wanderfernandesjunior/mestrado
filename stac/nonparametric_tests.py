...
# Ajustado por Ricardo Vargas
#def bonferroni_dunn_test(ranks, control=None):
def bonferroni_dunn_test(k, values, keys, control_i):
    """
    ...
    """
    # Ajustado por Ricardo Vargas por acreditar que ".index" não deveriam ser utilizados e pela explicação do parâmetro pivots estar bem ruim
    #k = len(ranks)
    #values = ranks.values()
    #keys = ranks.keys()
    #if not control :
...
def holm_test(k, values, keys, control_i):
    """
